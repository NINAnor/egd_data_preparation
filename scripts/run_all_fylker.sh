...
#
# Usage:  bash scripts/run_all_fylker.sh [JOBS]
#   JOBS = number of concurrent fylke jobs (default 8). Each job uses `nthreads`
#   DuckDB threads and `memlimit` GB as set in R/gk_config.R, so keep
...
Rscript -e 'suppressPackageStartupMessages(library(fs)); source("R/gk_config.R"); cat(dir_ls(gdb_folder, regexp = "gdb$"), sep = "\n")' \
  | parallel -j "$JOBS" --joblog "$LOG_DIR/joblog.tsv" --results "$LOG_DIR/{/.}/" \
      Rscript scripts/run_fylke.R {}
